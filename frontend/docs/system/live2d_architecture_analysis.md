...
### 4.4 音频律动 (Audio Rhythm)
由 [`src/composables/useRhythm.js`](src/composables/useRhythm.js:4) 实现，通过 Web Audio API 分析系统或麦克风音频，驱动模型动作。
...
**解析**：
- 这是一个高度定制的音频可视化驱动器。
- 提取音频的低频（Bass）和中频（Mid）能量。
- 实现了节拍检测（Beat Detection），在检测到重音时触发头部点头（`ParamAngleZ`）。
- 结合正弦波和音频能量，计算身体摇摆（`ParamBodyAngleX`）和呼吸（`ParamBreath`），使模型随音乐自然律动。
- 包含开机动画（Boot Sequence）和随机的故障抖动（Glitch）效果。
