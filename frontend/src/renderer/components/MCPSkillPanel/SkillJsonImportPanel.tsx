/**
 * MCP Skill JSON 批量导入面板。
 *
 * 做什么：提供 JSON 导入方式注册 MCP Skill，支持文件上传和直接粘贴。
 *         三阶段流程：输入 JSON → 解析预览 → 导入结果。
 * 为什么这样做：与 MCP 本地服务器的 JSON 导入模式保持一致。
 * 输入输出：用户导入 JSON 配置后调用批量注册 API。
 * 边界条件：支持 .json 文件上传和文本粘贴两种方式。
 * 异常行为：JSON 解析失败时显示详细错误信息。
 */
import React, { useState, useRef, useCallback } from 'react';
import { useSkillStore } from '../../stores/mcpSkillStore';
import { parseSkillJSON, ImportResultSummary } from './skillJsonParser';
import type { SkillConfig } from '../../services/mcpSkillService';

type ImportStage = 'input' | 'preview' | 'result';

export const SkillJsonImportPanel: React.FC = () => {
  const [jsonText, setJsonText] = useState<string>('');
  const [parsedRows, setParsedRows] = useState<SkillConfig[]>([]);
  ...
  const { batchRegisterSkills, importStatus, importResult, resetImportStatus } =
    useSkillStore();
  ...
  const handleParse = useCallback(() => {
    setParseError('');
    try {
      const result = parseSkillJSON(jsonText);
  ...
  const handleConfirmImport = useCallback(async () => {
    resetImportStatus();
    try {
      await batchRegisterSkills(parsedRows);
      setStage('result');
    } catch {
      // 错误已在 Store 中处理
    }
  }, [parsedRows, batchRegisterSkills, resetImportStatus]);
