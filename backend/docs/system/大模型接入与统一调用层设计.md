...
本文档旨在为 Luna 项目设计并规范 **大模型接入与统一调用层（LLM Access and Unified Invocation Layer）**。该层位于 Python AI Service 的底层基础设施侧，向上为 Python 侧的推理逻辑（Agent、记忆、工具调度等模块）提供纯粹的、无状态的模型调用能力。
...
* **输出不稳定性**：即使使用强大的模型，也可能出现 JSON 格式破损，导致 Python 侧的结构化解析崩溃。
* **状态机阻塞**：如果大模型调用卡死且无超时限制，将导致 Python 后端中的 asyncio Task 长期挂起，耗尽资源。
...
| **Python API 层**        | 接收 Electron 前端的请求，通过 Agent 层调度 LLM 调用。  | **禁止**直接修改 Prompt 业务内容；透传前端 Cancel 信号进行中断。          |
...
sequenceDiagram
    ...
    participant LLM_Layer as LLM Invoker
    participant Resilience as Circuit Breaker & Retry
    participant OAI as OpenAI Compatible API

    Electron->>PyAPI: WebSocket/SSE: 发送用户消息
    PyAPI->>Agent: 转发请求并分配 TraceID
    Agent->>Agent: 组装 Full Prompt & JSON Schema
    Agent->>LLM_Layer: invoke(LLMInvokeRequest)
    ...
    LLM_Layer-->>Agent: 返回 LLMInvokeResponse
    Agent-->>PyAPI: 提取结果回传
    PyAPI-->>Electron: WebSocket/SSE 流式推送结果
```
...
* **Python API 层**：通过 asyncio 传播超时信号与取消信号。前端点击"停止生成"时，Python 端通过 WebSocket 接收 Cancel 指令，必须捕获 `asyncio.CancelledError`，断开与 OpenAI 的 TCP 连接。
* **Agent Cognitive Logic**：提供上层包装，例如 `summarize_memory(text)` 会调用 `invoke` 并传入 `{"summary": "str", "entities": ["str"]}` 的 JSON Schema。
* **状态机模块**：Python 侧的 LangGraph 工作流在编排执行过程中，将 LLM 调用封装为 Node，通过 Invoker 获取结果后驱动图状态流转。
...
3. **Trace ID 透传**：Python API 入口处生成的 `TraceID` 必须贯穿整个 LLM 调用链路，记录到日志上下文中，确保全链路审计。
...
* **场景 1：主动行为规划 (Sync + JSON)**
  Python Agent 层被 LangGraph 工作流唤醒，要求基于当前 Memory 进行今日工作总结。Python 层组装好 Prompt 和 `SummarySchema`，调用 `invoke()` 阻塞获取结构化结果，处理完成后驱动 LangGraph 状态继续流转。
* **场景 2：语音聊天对话 (Stream + Text)**
  用户对桌面宠物说话，Electron 前端通过 WebSocket 透传给 Python API，Python 层调用 `stream_invoke()`，每次 `yield` 的文本切片通过 SSE 流推回前端，前端立刻转发给 TTS 引擎发声。
...
3. **流式中断的错误处理**：流式返回中，如果连接中断（如 VPN 切换），可能抛出不完整的迭代。
   * **规避**：前端（Electron）必须依赖完整的结束标识符来确认状态，Python 后端将其标记为中断并通知前端进入错误恢复。
