...
CREATE TABLE long_term_memories (
    id VARCHAR(64) PRIMARY KEY,       -- 雪花算法 ID（Snowflake），与 Qdrant Point ID 对应
...
- **降级策略**：
  ...
  2. 强制将 Payload 写入本地文件系统（`/luna_workspace/blobs/<snowflake_id>.dat`）。
  3. 数据库和 Redis 中仅存储其引用地址 (`file://blobs/<snowflake_id>.dat`)。
...
def upsert_memory(memory_id: str, agent_id: str, text: str, score: int):
    try:
        ...
        client.upsert(
            ...
            points=[
                PointStruct(
                    id=memory_id,  # 雪花算法 ID（Snowflake）
                    vector=vector,
                    payload={
                        "agent_id": agent_id,
...
1. **Qdrant ID 必须统一使用雪花算法 ID**
   - *问题*：Qdrant 的 Point ID 需要与主存数据库中的 ID 字段严格对应。如果使用不同类型的 ID 格式，会导致检索查询时无法对齐。
   - *规避*：Go 生成 ID 时，使用雪花算法生成器（`internal/utils/snowflake`），并通过 `GenerateStringID()` 转为字符串后作为 Qdrant Point ID。
